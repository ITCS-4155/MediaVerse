...
export default function ExplorePage() {
    const [activeId, setActiveId] = useState("movie");
    ...
    useEffect(() => {
        const fetchTrending = async () => {
            ...
            try {
                const res = await fetch(`/media/search?type=${activeId}`);
                const data = await res.json();

                if (data.results) {
                    setTrendingItems(data.results.slice(0, 6));
                } else {
                    setTrendingItems([]);
                }
            } catch (error) {
            ...
            <div className="relative z-10 max-w-7xl mx-auto px-6 pt-24 pb-20">
                ...
                <div className="mb-14">
                    <div className="flex items-center justify-between mb-5">
                        ...
                        <button onClick={() => navigateToSearch()} className="flex items-center gap-1 text-xs transition-colors duration-300 hover:opacity-70"
                    ...
                    {isLoadingTrending ? (
                        <div className="flex justify-center py-10 border border-dashed rounded-2xl" style={{ borderColor: current.border }}>
                            <span className="text-sm tracking-widest uppercase animate-pulse" style={{ color: current.accent }}>Loading trending {current.label.toLowerCase()}...</span>
                        </div>
                    ) : trendingItems.length === 0 ? (
                        <div className="flex justify-center py-10 border border-dashed rounded-2xl" style={{ borderColor: current.border }}>
                            <span className="text-sm tracking-widest" style={{ color: current.sub }}>No trending items found.</span>
                        </div>
                    ) : (
                ...
                {/* Featured card for current category */}
                <div className="mb-14">
