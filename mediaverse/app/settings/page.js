        ...
        async function fetchSettings() {
            try {
                ...
                if (data?.user?.id) {
                    ...
                    const fullUser = profileRes.success ? profileRes.data : data.user;

                    setProfile({
                        name: fullUser.name || "",
                        location: fullUser.location || "",
                        bio: fullUser.bio || "",
                        avatar: fullUser.image || null,
                    });
                    setAccount((prev) => ({ ...prev, email: fullUser.email || "" }));
                }
            } catch (err) {
    ...
    const handleSave = async () => {
        ...
        try {
            if (activeTab === "profile") {
                ...
                const res = await updateAccount(userId, account);
                if (res.error) throw new Error(res.error);
            ...
            <div style={styles.layout}>
                ...
                <main style={styles.main}>
                    <div style={styles.topBar}>
                        <Link href="/profile" style={styles.backBtn}>
                    ...
                    <div style={styles.saveBar}>
                        <button onClick={handleSave} disabled={isSaving} style={{...styles.saveBtn, opacity: isSaving ? 0.7 : 1}}>
                            {isSaving ? "Saving..." : saved ? <><CheckIcon /> Saved</> : "Save changes"}
                        </button>
                    </div>
                </main>
            </div>
...
const styles = {
    ...
    topBar: { display: "flex", justifyContent: "flex-end", marginBottom: "1.5rem" },
    backBtn: { color: "#6ee7b7", textDecoration: "none", fontSize: "0.9rem", fontWeight: "600", transition: "opacity 0.2s" },
    ...
    toggleDesc: { margin: "0 0 0.8rem", color: "#6b7280", lineHeight: 1.4 },
